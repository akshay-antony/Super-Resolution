...
class RangeToPCD:
    def __init__(self,
        ...
        self.ang_res_y = fov_y / float(image_rows-1) # vertical resolution
        ...
        self.elevation_image = np.float32(elevation_image * self.ang_res_y) - self.ang_start_y
        ...
        self.elevation_image = torch.from_numpy(self.elevation_image).to(device)
        self.azimuth_image = torch.from_numpy(self.azimuth_image).to(device)
        self.intensity = elevation_image + azimuth_image / image_cols

    def convert_range_to_pcd(self,
                             range_image,
                            ):
        range_image = range_image.squeeze(1)
        range_image = range_image * self.max_range
