...
class Network(nn.Module):
	def __init__(self,image_height=16,image_width=1800):
		...
		self.upblock = upBlock(64*16, 64*8, (2,2), (1,0), (1,0))
		
		self.decoder1 = nn.Sequential(
							conv(1024, 64*8),
							nn.Dropout(p=0.25),
							upBlock(512, 64*4, (2,2), (1,1), (1,1)))
		
		self.decoder2 = nn.Sequential(
							conv(512, 64*4),
							nn.Dropout(p=0.25),
							upBlock(256, 64*2, (2,2), (1,1), (1,1)))
	...
	def forward(self, x):
		...
		y4 = self.upblock(y4)
		y3 = torch.cat((x4, y4), axis=1)
		y3 = self.decoder1(y3)
		y2 = torch.cat((x3, y3), axis=1)
		y2 = self.decoder2(y2)
		y1 = torch.cat((x2, y2), axis=1)
		y1 = self.decoder3(y1)
		y0 = torch.cat((x1, y1), axis=1)
